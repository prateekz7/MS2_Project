...
/**
 * Helper class containing static method for dealing with numbers
 * @author Sarit.
 */
public class Numbers {
     ...
     * For array of 2 bytes the result is signed. for 1 byte it is unsigned.
     ...
     * @return - iny with the parsed value or 0 if the input array is null/empty or the input length is wrong
     */
     ...
     * @param length - int for number of bytes to parse
     * @param byteOrder - ByteOrder object
     * @param error - arry of boolean with allocation for at least one member for error return instead of using exception
     * @return - iny with the parsed value or 0 if the input array is null/empty or the input length is wrong
     */
    ...
    /**
     * Convert array of bytes to a Long integer.
     * For array of 2 bytes the result is signed. for 1 byte it is unsigned.
     * @param src - byte array to be parsed
     * @param startIndex - int for offset into the byte array
     * @param length - int for number of bytes to parse
     * @param byteOrder - ByteOrder object
     * @return - long with the parsed value or 0 if the input array is null/empty or the input length is wrong
     */
     ...
     * @param byteOrder - ByteOrder object
     * @param error - arry of boolean with allocation for at least one member for error return instead of using exception
     * @return - short with the parsed value or 0 if the input array is null/empty or the input length is wrong
     */
    public static long parseLong(byte[] src, int startIndex, int length, ByteOrder byteOrder, boolean[] error) {
        ...
        if (byteOrder.equals(ByteOrder.BIG_ENDIAN)) {
            for (int i = (startIndex + length-1); i>=startIndex; i--) {
                result |= ((long)(src[i] & 0xff) << shift);  // must cast to long since the default when signed shifting is int (4 bytes)
                shift += 8;
            }
        }
        else {
    ...
    /**
     * Convert array of bytes to a short integer.
     * For array of 2 bytes the result is signed. for 1 byte it is unsigned.
     * @param src - byte array to be parsed
     * @param startIndex - int for offset into the byte array
     * @param length - int for number of bytes to parse
    ...
    public static int GetSignedShortFromUnsignedInt(int source) {
        ...
        //byteBuffer.order(ByteOrder.BIG_ENDIAN); // optional, the initial order of a byte buffer is always BIG_ENDIAN.
